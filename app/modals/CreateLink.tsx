...
export default function CreateLink({ enable, setEnable }: Props) {
    ...
    const [meetHubLink, setMeetHubLink] = useState<string>("");
                        ...
                        <div className='flex min-h-full items-center justify-center p-4 text-center'>
                            ...
                            >
                                <DialogPanel className='w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white p-6 align-middle shadow-xl transition-all text-center'>
                                    {showMeetingLink ? (
                                        <MeetingLink meetHubLink={meetHubLink} />
                                    ) : (
                                        <MeetingForm
                                            setShowMeetingLink={setShowMeetingLink}
                                            setMeetHubLink={setMeetHubLink}
                                        />
                                    )}
                                </DialogPanel>
                            </TransitionChild>
...
const MeetingForm = ({
    ...
    setMeetHubLink: Dispatch<SetStateAction<string>>;
}) => {
    ...
    return (
        <>
            ...
            <Description className='text-xs opacity-40 mb-4'>
                Schedule a MeetHub meeting with your click
            </Description>
            ...
            <form className='w-full' onSubmit={handleStartMeeting}>
                ...
                <input
                    ...
                    className='mt-1 block w-full text-sm py-3 px-4 border-gray-200 border-[1px] rounded mb-3 text-gray-800'
                    required
...
const MeetingLink = ({ meetHubLink }: { meetHubLink: string }) => {
    ...
    return (
        <>
            ...
            <Description className='text-xs opacity-40 mb-4'>
                You can share the MeetHub link with your participants
            </Description>

            <div className='bg-gray-100 p-4 rounded flex items-center justify-between'>
                <p className='text-xs text-gray-500'>
                    {`${process.env.NEXT_PUBLIC_MEETHUB_HOST}/${meetHubLink}`}
                </p>

                <CopyToClipboard
                    onCopy={handleCopy}
                    text={`${process.env.NEXT_PUBLIC_MEETHUB_HOST}/${meetHubLink}`}
                >
